...
class SequenceCutPaste:
    def __init__(self, object_dir, paste_max_obj_num):
        ...
        self.sub_labels = (5, 4, 1, 8, 3, 6, 2, 7)
        '''
        other-vehicle: 7014
        bicycle: 4063
        motorcyclist: 530
    ...
    def paste_single_obj(self, pcds_list, pcds_label_list, pcds_label_single_list, pcds_road_list, pcds_raw_label_list):
        '''
        Input:
        ...
        if(len(pc_object_list[0][0]) < 10):
            return pcds_list, pcds_label_list, pcds_label_single_list, pcds_raw_label_list
        ...
        for theta in theta_list:
            ...
            if valid_flag:
                ...
                for ht in range(len(pc_object_aug_list)):
                    ...
                    assert pcds_label_single_list[ht].shape[0] == pcds_label_list[ht].shape[0]
                    _, fov_mask = valid_position_list[ht]

                    pcds_filter_ht = pcds_list[ht][~fov_mask]
                    pcds_label_filter_ht = pcds_label_list[ht][~fov_mask]
                    pcds_label_single_filter_ht = pcds_label_single_list[ht][~fov_mask]
                    ...
                    pcds_addobj_single_label_ht = np.full((pcds_obj_aug_ht.shape[0], ), fill_value=paste_label, dtype=pcds_label_single_filter_ht.dtype)
                    ...
                    pcds_label_single_list[ht] = np.concatenate((pcds_label_single_filter_ht, pcds_addobj_single_label_ht), axis=0)
    ...
    def __call__(self, pcds_list, pcds_label_list, pcds_label_single_list, pcds_road_list, pcds_raw_label_list):
        paste_obj_num = random.randint(0, self.paste_max_obj_num)
        if paste_obj_num == 0:
            return pcds_list, pcds_label_list, pcds_label_single_list
        else:
            for i in range(paste_obj_num):
                pcds_list, pcds_label_list, pcds_label_single_list, pcds_raw_label_list = self.paste_single_obj(
                    pcds_list, pcds_label_list, pcds_label_single_list, pcds_road_list, pcds_raw_label_list)
