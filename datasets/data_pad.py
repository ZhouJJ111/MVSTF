...
class DataloadTrain(Dataset):
    ...
    def form_seq(self, meta_list):
        ...
        return pc_list, pc_label_list, pc_label_single_list, pc_road_list, pc_raw_label_list
    ...
    def __getitem__(self, index):
        ...
        if random.random() > 0.5:
            pc_list, pc_label_list, pc_label_single_list, pc_road_list, pc_raw_label_list = self.form_seq(meta_list)
        else:
            pc_list, pc_label_list, pc_label_single_list, pc_road_list, pc_raw_label_list = self.form_seq(meta_list_raw)
        
        # copy-paste
        if self.cp_aug is not None:
            pc_list, pc_label_list, pc_label_single_list = self.cp_aug(pc_list, pc_label_list, pc_label_single_list, pc_road_list, pc_raw_label_list)
        ...
        for ht in range(len(pc_list)):
            ...
            pc_label_single_list[ht] = np.pad(pc_label_single_list[ht], ((0, pad_length),), 'constant', constant_values=0)
...
class DataloadVal(Dataset):
    ...
    def __getitem__(self, index):
        meta_list, meta_list_raw = self.flist[index]
        _, _, _, seq_id, file_id = meta_list[0]                     # modify

        # # modify
        # seq_id = self.seq_ids[index]  # 读取序列号
        # idx = self.indices[index]  # 读取索引


        # load history pcds
        pc_list, pc_label_list, pc_label_single_list = self.form_seq(meta_list_raw)

        point_num = pc_list[0].shape[0]                             # modify
        ...
        # ic('zjj:', point_num, valid_mask_list[0].shape, pad_length_list[0])

        return pcds_xyzi, pcds_coord, pcds_sphere_coord, \
               pcds_target, pcds_target_single, \
               valid_mask_list, pad_length_list, meta_list_raw, \
               seq_id, file_id, point_num
...
class DataloadTest(Dataset):
    def __init__(self, config):
        ...
        seq_num = config.seq_num
        # add training data
        seq_split = [str(i).rjust(2, '0') for i in self.task_cfg_mos['split']['test']]
        for seq_id in seq_split:
            fpath = os.path.join(config.SeqDir, seq_id)
            fpath_pcd = os.path.join(fpath, 'velodyne')
            # fpath_label = os.path.join(fpath, 'labels')
            ...
            for i in range(len(poses_list)):
                ...
                elif i > (len(poses_list) - seq_num):           # i > sum - 9
                    # forward
                    for ht in range(seq_num):
                        file_id = str(i - ht).rjust(6, '0')
                        fname_pcd = os.path.join(fpath_pcd, '{}.bin'.format(file_id))
    ...
    def __getitem__(self, index):
        meta_list, meta_list_raw = self.flist[index]
        _, _, seq_id, file_id = meta_list[0]  # modify
        ...
        for ht in range(len(pc_list)):
            ...
            # pc_label_list[ht] = np.pad(pc_label_list[ht], ((0, pad_length),), 'constant', constant_values=0)
            # pc_label_single_list[ht] = np.pad(pc_label_single_list[ht], ((0, pad_length),), 'constant',
            #                                   constant_values=0)
            pad_length_list.append(pad_length)
        ...
        ic('zjj:', seq_id, file_id)
        # ic('zjj:', pcds_xyzi.shape, pcds_coord.shape, pcds_sphere_coord.shape,
        #    pad_length_list, meta_list_raw, seq_id, file_id, point_num)

        return pcds_xyzi, pcds_coord, pcds_sphere_coord, pad_length_list, meta_list_raw, seq_id, file_id, point_num
               # pcds_target, pcds_target_single, valid_mask_list,
