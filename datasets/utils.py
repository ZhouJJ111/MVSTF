...
def Quantize(pcds, range_x=(-40, 62.4), range_y=(-40, 40), range_z=(-3, 5), size=(512, 512, 20)):
    ...
    pcds_quan = np.stack((x_quan, y_quan, z_quan), axis=-1)     # (1170000,3), 3: x_quan, y_quan, z_quan
    return pcds_quan


def SphereQuantize(pcds, phi_range=(-180.0, 180.0), theta_range=(-16.0, 10.0), size=(64, 2048)):    # (1170000,4)
    H = size[0]
    W = size[1]
    
    phi_range_radian = (phi_range[0] * np.pi / 180.0, phi_range[1] * np.pi / 180.0)            # (-PI,PI)
    theta_range_radian = (theta_range[0] * np.pi / 180.0, theta_range[1] * np.pi / 180.0)
    
    dphi = (phi_range_radian[1] - phi_range_radian[0]) / W                                     # (2 * PI)/2048
    dtheta = (theta_range_radian[1] - theta_range_radian[0]) / H
    
    x, y, z = pcds[:, 0], pcds[:, 1], pcds[:, 2]
    d = np.sqrt(x ** 2 + y ** 2 + z ** 2) + 1e-12                            # 深度值(距离)
    
    phi = phi_range_radian[1] - np.arctan2(x, y)                             # 待修改 np.arctan2(y, x)
    phi_quan = (phi / dphi)
    
    theta = theta_range_radian[1] - np.arcsin(z / d)
    theta_quan = (theta / dtheta)
    
    sphere_coords = np.stack((theta_quan, phi_quan), axis=-1)          # (1170000,2), 2:theta_quan, phi_quan
