...
def val_fp16(epoch, model, test_loader, category_list, category_list_single, save_path, rank=0):
    ...
    file = open(os.path.join(save_path, 'infer_test_fp16_{}.txt'.format(rank)), 'a')
    ic(file)

    with torch.no_grad():
        runtime = []
        for i, (pcds_xyzi, pcds_coord, pcds_sphere_coord,
                pad_length_list, meta_list_raw,
                seq_id, file_id, point_num) in tqdm.tqdm(enumerate(test_loader)):
            # ic('zjj:', i, pcds_xyzi.shape, pcds_coord.shape, pcds_sphere_coord.shape)       # [1, 4, 9, 7, 160000, 1], [1, 4, 9, 160000, 3, 1], [1, 4, 9, 160000, 2, 1]
            ...
            save_mos_path = os.path.join(save_path, 'test11_21', 'MOS_sequences', seq_id_str, 'predictions', file_id_str_with_label)
            ...
            # ic(i, seq_id, file_id, point_num)
            ic(pred_single.shape)
            # ic(type(pred_single), pred_single)

            seq_id_str = seq_id[0] if isinstance(seq_id, list) else seq_id
            file_id_str = file_id[0] if isinstance(file_id, list) else file_id
            file_id_str_with_label = f"{file_id_str}.label"  # 后面加上 .label
            save_single_path = os.path.join(save_path, 'test11_21', 'Single_sequences', seq_id_str, 'predictions', file_id_str_with_label)
...
def main(args, config):
    ...
    for epoch in range(args.start_epoch, args.end_epoch + 1, world_size):
        if (epoch + rank) < (args.end_epoch + 1):
            ...
            if pGen.fp16:
                val_fp16(epoch + rank, model, test_loader, pGen.category_list, pGen.category_list_single, model_prefix, rank)
            else:
                val(epoch + rank, model, test_loader, pGen.category_list, model_prefix, rank)
