...
class RVNet(nn.Module):
    def __init__(self, base_block, context_layers, layers, use_att):
        ...
        self.header = self._make_layer(eval('backbone.{}'.format(base_block)), context_layers[0], context_layers[1],
                                       layers[0], stride=(1, 2), dilation=1, use_att=use_att)

        # MAFL
        # self.header2 = MAFL(in_channel=context_layers[1], middle_channel=context_layers[2],
    ...
    def forward(self, x):
        ...
        x1 = self.res1(x02)
        x2 = self.res2(x1)
        ic('RVNet_Input:', x.shape, x1.shape, x2.shape)
        ic('RVNet_MAFL:', x0.shape, x02.shape)
...
class RVNet_new(nn.Module):
    def __init__(self, base_block, context_layers, layers, use_att):
        super(RVNet_new, self).__init__()
        #encoder
    ...
    def forward(self, x):
        #pdb.set_trace()
        #encoder                        #  x:[1,64,64,2048]
        x0 = self.header(x)             # x0:[1,32,64,1024], 1/2
        x1 = self.res1(x0)              # x1:[1,64,32,512],  1/2
        x2 = self.res2(x1)              # x2:[1,128,16,256], 1/2
        
        #decoder
        x_merge1 = self.up2(x1, x2)         # x_merge1:[1,96,32,512],  *2
        x_merge = self.up1(x0, x_merge1)    #  x_merge:[1,64,64,1024], *2
        return x_merge
