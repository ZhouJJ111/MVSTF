...
# 参见CPGNet: dual down-sampling block
class DownSample2D(nn.Module):
    ...
    def forward(self, x):                   #      x:[1,64,512,512]
        x_conv = self.conv_branch(x)        # x_conv:[1,32,256,256]
        x_pool = self.pool_branch(x)        # x_pool:[1,32,256,256]
        x_out = self.act(x_conv + x_pool)   #  x_out:[1,32,256,256]
        return x_out
...
# https://blog.csdn.net/qq_41204464/article/details/126881984
class ChannelAtt(nn.Module):
    def __init__(self, channels, reduction=4):
        super(ChannelAtt, self).__init__()
        self.cnet = nn.Sequential(
                nn.AdaptiveAvgPool2d(output_size=1),    # C*1*1
...
class SpatialAtt(nn.Module):
    def __init__(self, channels, reduction=4):
        ...
        self.snet = nn.Sequential(
                conv3x3(channels, 4, stride=1, dilation=1),             # 4*H*W
                nn.BatchNorm2d(4),
                act_layer,
                conv3x3(4, 1, stride=1, dilation=1, bias=True),         # 1*H*W 
                nn.Sigmoid()
            )
...
class CSAtt(nn.Module):
    ...
    def forward(self, x):
        #channel wise
        x1 = self.channel_att(x)        # 不同通道分配不同的权重
        x2 = self.spatial_att(x1)       # 不同位置分配不同的权重
        return x2
...
# 空间注意力(可迁移)： input.size() = output.size()
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, reduction=1, dilation=1, use_att=True):
        ...
        if self.use_att:
            # self.channel_att = ChannelAtt(channels=inplanes, reduction=4)       # 不同通道分配不同的权重
            self.attn = SpatialAttention(inplanes)
...
class PointNet(nn.Module):
    ...
    def forward(self, x):           # input: x[9,7,130000,1]          [1,15,512,512]    [1,64,512,512]
        x_feat = self.layer(x)      # x_feat:x[9,64,130000,1]
        return x_feat


class PointNetStacker(nn.Module):
    # 超参数：7, rv_context_layer[0], pre_bn=True, stack_num=2
...
# 融合[point,BEV,RV]: 3*[BS,64,N,1] -----> [BS,64,N,1]
class CatFusion(nn.Module):
    ...
    def forward(self, *x_list):             # x_list:3*[1,64,130000,1]
        x_merge = torch.cat(x_list, dim=1)  # x_merge: [1,192,130000,1]
        x_merge = F.dropout(x_merge, p=0.2, training=self.training, inplace=False)
        x_out = self.merge_layer(x_merge)   # x_out:[1,64,130000,1]
        return x_out
...
if __name__ =='__main__':
    input1 = torch.randn(1,64,512,512)
    coord1 = torch.randn(1,1000,2,1)
    input2 = torch.randn(1,64,64,1024)
    coord2 = torch.randn(1,1000,2,1)
    model1 = BilinearSample(64,(0.5,0.5))
    model2 = BilinearSample(64,(1.0,0.5))
    out1 = model1(input1,coord1)    
    out2 = model2(input2,coord2)
    print(out1.size(),out2.size())      # [1, 64, 1000, 1]