...
def flatten_probas(probas, labels, ignore=None):        # input:(1,3,130000,1) (1,130000,1)
    """
    ...
    if probas.dim() == 3:
        ...
        B, C, H, W = probas.size()
        probas = probas.permute(0, 2, 3, 1).contiguous().view(-1, C)  # (1,130000,1,3) --->(130000,3)  B * H * W, C = P, C
        labels = labels.view(-1)                                      # (130000,)
    else:
        assert probas.dim() == 2
    
    if ignore is None:
        return probas, labels
    valid = (labels != ignore)                                         # 去除 ignore=0 的点云
    vprobas = probas[valid.nonzero().squeeze()]                        # (N,3)
    vlabels = labels[valid]                                            # (N,)
