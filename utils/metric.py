...
class MultiClassMetric:
    ...
    def addBatch(self, gt, pred):
        ...
        _, pred_map = torch.max(pred, dim=1)                 # 查看tensor:pred_map.data.cpu().numpy()
    ...
    def get_metric(self):
        ...
        iou = self.tp / (self.gt_num + self.pred_num - self.tp + 1e-12)         # IoU = TP/(TP+FP+FN)
        pre = self.tp / (self.pred_num + 1e-12)                                 # Precision = TP/(TP+FP)
        rec = self.tp / (self.gt_num + 1e-12)                                   # Recall = TP/(TP+FN)

        '''                                   
                    prediction: static    moving  
        ground truth: static  [  TP         FN   ]  
                      moving  [  FP         TN   ]  
        '''
